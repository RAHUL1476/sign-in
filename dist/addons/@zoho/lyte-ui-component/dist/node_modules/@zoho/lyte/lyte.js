...
Lyte.Globals.set = function(scope,value){
 ...
 Lyte.Globals.get = function(scope){
  ...
  Lyte.extendService = function(arg){
      var servObj = Lyte.toBeUsedServices(arg) , name;
      for(var serv in servObj){
          name = servObj[serv];
          if(Lyte.registeredServices.hasOwnProperty(name)){
              var ins = arg.ins, service = Lyte.registeredServices[name];
	  ...
	  /*-----------string declaration starts------------*/
		...
		run[getResourcesStr] = function(hook,index) {
			...
			if(callback && validateTransition(this)) {
				...
				r.__lp[reqestedType[hook]+'Loaded'] = !require || !(require.length || typeof require == "object")
				if(require) {
		...
		function assignServices(options,fns,dir,def) {
			Lyte.extendService({services : options && options.services || [], type:"route", ins: def || getFnsAndIns(dir,fns), callback : function(serv, key, aName, serName){
				var obj = {};
				obj[key] = serName;
...
class customElementPrototype extends elementPrototype{
    constructor() {
        ...
        this.constructor.compInstances.push(this);
        let fastRenderIndex;
    ...
    afterConnected() {
        ...
        if(this._initProperties) {
            ...
            for(let key in initProperties) {
                ...
                if(!error) {
                    compData[key] = actVal;    
                     if(field && /^(object|array)$/.test(field.type) && field.watch == true){
    ...
    disconnectedCallback() {
        ...
        if(scpObj){
            for(var key in scpObj){
                var propData = this.component.data[key], id = scpObj[key];
                if(propData){
...
//                                          this.establishBindings(item._propBindingObject, value);
        ...
        "callDeepObservers" : function (data, args, property){
            var type = args.type, self = this;
            data._scp.forEach(function(scp, id){
                var pathArr = Object.keys(scp), aPath;
                if(pathArr.length == 1){
                    aPath = pathArr[0];
                }
                else if(pathArr.length > 1){
...
Lyte.$.injectServices.component = function(key, name){
    ...
    if(registeredComponents){
        for(var comp in registeredComponents){
            var component = registeredComponents[comp], obj = {};
            obj[key] = name;
...
Object.defineProperties($Record.prototype,{
	...
	persist:{
		value: function(obj){
			var mName = this.model._name, rec = this.record, partialObj = {obj:new Map()};
			if(this.isNew){
...
var store = {
	...
	$:{ 
		...
		getFromCB: function(type, name, key){
				var scp = store[type], obj = scp ? scp[name] : undefined;
		...
		extendService : function(services, type, name, self, scope){
			Lyte.extendService({services: services, type:type, name:name, ins: self, callback : function(serv, key, name){
		...
		cbScp:function(name, key, type){
			var scope = store[type], layer = scope[name], callback, application = scope.application;
		...
		addDeepNest:function(record, extended, type){
			var relatedRecord;
		...
		removeSelfCircularReference : function(modelName, obj, expose, type, partialObj, partialRef){
			...
			var polymorphicType = obj && obj.$ ? obj.$.polymorphicType : undefined, refId;
			if(partialRef){
		...
		toJSONObj : function(model, data, expose, type, partial){
		...
		insertIntoStore:function(modelName,data,saveParent,stack,partialObj){
			var ret;
			if(Array.isArray(data)){
				ret = [];
				for(var i=0; i<data.length; i++){
		...
		removeFromStore:function(model,keys,fromStore,ignorePartial, delayPer, onlyRem){
			var data = model.data;
			if(data.length == 0){
				return;
			}
		...
		establishLink:function(forward,backward,data,relatedRecord,index,ignorePartial, isDuplicate){
			...
			var fRelKey = forward.relKey, isPoly = forward.opts && forward.opts.polymorphic, type = isPoly ? relatedRecord._type : undefined;
			if(forward.relType == "belongsTo"){
		...
		toDemolishRelation:function(model,index,ignorePartial,onlyRem,delayPers){
		...
		toDemolishLink:function(model,record,relation,ignorePartial,onlyRem,delayPers){
		...
		demolishLink : function(record, priKey, relatedRecord, bRelKey, relation, bRelation, ignorePartial, ignoreAttrCheck, onlyRem, delayPers){
...
store.adapter = {
	$: {
		buildURL : function(type,method,modelName,key, snapshot, queryParams,actionName,customData,url){
			var adapter = store.adapter[modelName], host = store.$.getFromCB("adapter",modelName,"host"), url = url || "", makeBatch = store.$.makeBatch;
			if(!url){
				if(!makeBatch){
					if(host !== undefined){
						url += host;
		...
		handleAjax:function(obj){
			...
			var qP = urlObj.qP, res = store.$.initCB("serializer", obj.modelName, "serialize", {args:["ajax",urlObj.data,undefined,obj.customData,obj.modelName,qP,undefined]});
			if(res){
				urlObj.data = res.data;
			}
		...
		removeDirtyStack : function(data){
			data.$._attributes = {};
        ...
        */
		...
		otherParseRequestPromise:function(resp,modelName,type,data,urlObj,xhr,partialObj,batchIndex,batch,resolve,reject,customData,partialRef){
