...
(function () {
	...
	Lyte.Globals.set = function (scope, value) {
		if (Lyte.__gl && !Lyte.__gl.hasOwnProperty(scope)) {
	...
	Lyte.Globals.get = function (scope) {
		return LyteComponent.get(Lyte.__gl, scope);
	};
	...
	Lyte.Service.register = function (name, service) {
		if (!Lyte._keywords.is(name)) {
			if (name.indexOf('-') != -1) {
				Lyte.warn('Hyphens cannot be used in serviceName');
			} else {
				var data = service(),
				    req;
				...
				if (serv && serv.length) {
					serv.forEach(function (obj) {
						obj.cB(data, obj.key, obj.name, name);
					});
				}
				return data;
			}
		} else {
			Lyte.warn('Service name should not be a keyword used in Lyte');
		}
	};
	...
	Lyte.$.requiredServices = function (key, serviceName, cB, type, name) {
		var reqServ = Lyte.requiredServices[serviceName];
		var req = reqServ ? reqServ : Lyte.requiredServices[serviceName] = [];
		req.push({ key: key, cB: cB, type: type, name: name });
	};
				...
				for (var key in itm) {
					if (!Lyte._keywords.is(key)) {
						if (!Lyte.toBeInjectedServices.hasOwnProperty(key)) {
							Lyte.toBeInjectedServices[key] = itm[key];
							serv[key] = itm[key];
						} else {
	...
	Lyte.extendService = function (arg) {
		...
		for (var serv in servObj) {
			...
			if (Lyte.registeredServices.hasOwnProperty(name)) {
				var ins = arg.ins,
				    service = Lyte.registeredServices[name];
				...
				Lyte.$.requiredServices(serv, name, arg.callback, arg.type, arg.name);
			}
		}
	};
	...
	Lyte.injectResources = function (files, every, completed, options) {
		...
		return new Promise(function (resolve, reject) {
			processRequirements(files, function () {
				console.log(successFiles, errorFiles);
		...
		function createScript(file, resolve) {
			...
			if (!Lyte.injectResources.availableTags[file] || Lyte.injectResources.availableTags[file].tag.tagName == "LINK") {
	...
	Lyte.establishObjectBinding = function (data, attr, fromStore, update) {
		...
		if (update && data._scp && data._scp.size) {
			data._scp.forEach(function (mpObj, id) {
		...
		if ((!fromStore || fld.watch) && data[attr]) {
	...
	Lyte.bindObj = function (data, key, id, path, mp, check) {
		mp = mp || new Map();
		var value = key != undefined ? data[key] : data;
		if (path.length > 1 && Lyte.nestScp[id].data === value) {
			if (Lyte.checkAndAddBind(value, id, path)) {
	...
	Lyte.checkAndAddBind = function (value, id, path) {
		if (!value._scp || !value._scp.size) {
			return true;
		}
		if (value._scp.size) {
			if (value._scp.has(id)) {
				var obj = value._scp.get(id);
	...
	Lyte.removeNestScp = function (value, id, mp, data) {
		mp = mp || new Map();
		if (Array.isArray(value)) {
			if (!mp.get(value)) {
				mp.set(value, true);
		...
		if (data) {
			Lyte.bindObj(data, undefined, id, [], undefined, true);
		}
	};
...
;(function (window) {
	function Router(routes) {
		...
		run[getDependenciesStr] = run[getResourcesStr] = function (hook, index) {
			var r = this.R[index],
			    ...
			    callback = r[hook];
			r.__lp[reqestedType[hook] + 'Loaded'] = true;
			if (callback && validateTransition(this)) {
				...
				r.__lp[reqestedType[hook] + 'Loaded'] = !require || !(require.length || (typeof require === 'undefined' ? 'undefined' : _typeof(require)) == "object");
				if (require) {
					if (hook == getDependenciesStr) {
						console.warn(dependencyRequestLog);
					}
					r.__lp[fnType] = function () {
						r.__lp[fnType] = pendingStr;
					};
		...
		run[beforeLoadStr] = function (hook, index) {
			callHookWithoutPromise.call(this, this.R[index][hook], this.routes[index], hook, index, params(index));
			return callHookWithPromise();
		};
		...
		function addToHistory(obj) {
			var type = obj.replace ? "replaceState" : "pushState",
			    url = config.history ? _delimit(shiftBaseURL(obj.url, true)) : '#' + (config.slashAfterHash ? _delimit(obj.url) : obj.url);
   ...
   */
		...
		function routeTransition(processed) {
			...
			newTransInfo = {
				replace: matched.replace || (matched.refreshRoute && currRoute == prevTrans.info.route ? true : matched.replace),
			...
			if (prevTrans) {
				...
				for (var i = 0, l = matched.route.length; i < l; i++) {
					...
					if (similar && route == prevMatched.route[i] && (!startFrom || tar != startFrom)) {
						...
						similar = false;
						if (templateToRemove == undefined && prevTrans.routes[i] && prevTrans.routes[i].__ltp.rendered) {
							templateToRemove = i;
						}
						unRendered.push(route);
					}
				...
				if (prevMatched.route.length > matched.route.length) {
					...
					if (templateToRemove == undefined && prevTrans.routes[index].__ltp.rendered) {
					...
					if (_typeof(routeInstance.forceFetch) == functionStr ? callHookWithoutPromise.call(this, routeInstance.forceFetch, routeInstance, "forceFetch", index, params(index)) : routeInstance.forceFetch) {
						...
						[redirectStr, renderTemplateStr, afterRenderStr].forEach(function (h) {
							...
							if (h == renderTemplateStr && transComp.unRendered[orgIndex + 1]) {
								runLoop.push({ hook: beforeLoadStr, index: orgIndex + 1 });
							}
						});
					} else {
		...
		function getRequirements(object) {
			...
			Lyte.injectResources(ins[reqType], every, function (successFiles, errorFiles) {
				if (!config.execRouteRequirementsOnDemand) {
					if (!this.aborted || trans.transComp.redirected && index <= trans.transComp.common.length - 1) {
				...
				defer: function (opt) {
					...
					fnType = hook == getDependenciesStr ? "loadDependencies" : "loadResources",
						...
						if (!errorFiles.length) {
							...
							if (trans.pending[reqType].has(index)) {
								trans.pending[reqType].delete(index);
								if (loc = trans.pending.forceFetch.has(index)) {
									trans.pending.forceFetch.delete(index);
									nestedForcedPromises.call(trans, trans.runLoop.forceFetch, trans.fRunningProm.resolve, trans.runLoop.forceFetch[index][0]);
								} else {
									nestedPromises.call(trans, trans.runLoop, "current", trans.runningProm.resolve);
					...
					if (ins[fnType] == 'pending') {
						if (!this.aborted || trans.transComp.redirected && index <= trans.transComp.common.length - 1) {
							injectJS(files, function (successFiles, errorFiles) {
								debugLog(reqType + ' after inject', this.transId, ins.objPath);
								delete ins[fnType];
		...
		var requirements = {
			get: function get(def, type) {
				var status = def.__lp[type + 'Loaded'] == true;
				var fnType = type == dependenciesStr ? "loadDependencies" : "loadResources";
				if (!def.__lp[fnType] || typeof def.__lp[fnType] != 'function' || status) {
					return status;
				} else {
					var err = def.__lp[fnType]();
					return err == false ? false : def.__lp[type + 'Loaded'] == true;
				}
			}
		};
			...
			if (validateTransition(this)) {
				...
				if (runLoop && runLoop.length) {
					...
					if (promise.hook == beforeModelStr) {
						cond = requirements.get(r, dependenciesStr);
					}
		...
		function transitionCompleted(obj) {
			...
			if (trans.running) {
				console.log('transitionCompleted', history.state);
				if (trans == newTrans) {
					newTrans = undefined;
				}
				if (newTransInfoClone) {
		...
		function setParamsInInst(processed) {
			...
			try {
				for (var i = 0, l = trans.R.length; i < l; i++) {
					...
					r = trans.routes[i];
					if (R.queryParams) {
						R.queryParams.forEach(function (key) {
							r.__ltp.queryParams[key] = matched.queryParams[key];
						});
					}
					r.__ltp.dynamicParam = matched.dynamicParams[i];
					if (i + 1 == l) {
						R.__lp.linkTags.forEach(function (tag) {
				...
				if (config.allLinks[matched.target]) {
					config.allLinks[matched.target].forEach(function (tag) {
		...
		Lyte.$.injectServices.router = function (key, name, type) {
			var obj = {};
			obj[key] = name;
			// Lyte.extendService({services : [obj], type:"route", ins: arr});
			assignServices({ services: [obj] }, undefined, undefined, getDefinitionArr());
		};

		function assignServices(options, fns, dir, def) {
			Lyte.extendService({ services: options && options.services || [], type: "route", ins: def || getFnsAndIns(dir, fns), callback: function callback(serv, key, aName, serName) {
					var obj = {};
					obj[key] = serName;
					Lyte.extendService({ services: [obj], type: "route", ins: dir ? getFnsAndIns(dir, fns) : getDefinitionArr() });
				} });
		}
		...
		function registerRoute(dir, fns, options) {
			...
			assignServices(options, fns, dir);
			fns.__lp = {
				options: options,
   ...
   */
		...
		var routePredefined = [getDependenciesStr, getResourcesStr, beforeModelStr, modelStr, afterModelStr, redirectStr, renderTemplateStr, afterRenderStr, beforeExitStr],
			...
			if (!processed.transComp || !processed.transComp.redirected) {
			...
			if (!prevInstance) {
				var paramChangeTrans = function paramChangeTrans(matched) {
 ...
 */
	...
	var customElementPrototype = function (_elementPrototype) {
		...
		function customElementPrototype() {
			...
			if (_this5.hasAttribute("lyte-rendered") || !_LyteComponent.registeredComponents[_this5.localName] || _this5._ccCalled) {
				_this5.__lyteIgnore = true;
				return _possibleConstructorReturn(_this5);
			}
			_this5.constructor.activeInstances++;
			_this5.constructor.compInstances.push(_this5);
			...
			if (!fastRenderedProp) {
				...
				var data = _this5.constructor._data ? _this5.constructor._data.apply(compInstance) : {};
				var def = "default";
				for (var _key5 in data) {
					var obj = data[_key5];
					compData[_key5] = obj[def];
					if (obj.watch == true && /^(object|array)$/.test(obj.type)) {
						Lyte.establishObjectBinding(compData, _key5, false);
					}
				}
				if (_this5.constructor._serviceToBeUsed) {
					for (var serKey in _this5.constructor._serviceToBeUsed) {
						var serName = _this5.constructor._serviceToBeUsed[serKey];
						if (!data.hasOwnProperty(serKey) && Lyte.registeredServices.hasOwnProperty(serName)) {
							compData[serKey] = Lyte.registeredServices[serName];
						}
					}
				}
				compData.errors = {};
				...
				if (this._initProperties) {
					...
					for (var _key8 in initProperties) {
						...
						if (!error) {
							...
							if (_field && /^(object|array)$/.test(_field.type) && _field.watch == true) {
          ...
          */
						...
						if (multiplePropNode[i]) {
							...
							if (node.ownerElement && (node.ownerElement.hasAttribute("lyte-for") || node.ownerElement.hasAttribute("lyte-if") || node.ownerElement.hasAttribute("lyte-switch") || node.ownerElement.hasAttribute("lyte-forin")) || node.ownerElement && node.ownerElement.tagName === "TEMPLATE" && /^(for|forIn)$/.test(node.ownerElement.getAttribute("is")) && !isHelper) {
								...
								if (type === "for" && node.nodeName === "items" || type === "forIn" && node.nodeName === "object") {
									...
									if (!_multipleProp.actProp._forHelpers.size) {
										delete _multipleProp.actProp._forHelpers;
									}
									if (!prop._forHelpers) {
          ...
          delete actData._bindings;
			...
			value: function disconnectedCallback() {
				...
				var scpObj = this.component.data._scpObj;
				if (scpObj) {
					for (var key in scpObj) {
						var propData = this.component.data[key],
						    id = scpObj[key];
						if (propData) {
							Lyte.removeNestScp(this.component.data[key], id);
						}
					}
				}
				if (!this._fR) {
				...
				var rmIndex = this.constructor.compInstances.indexOf(this);
				this.constructor.compInstances.splice(rmIndex, 1);
			}
		}], [{
			key: 'updateValue',
   ...
   */
		...
		"registerComponent": function registerComponent(componentName, definition, options) {
			...
			for (var key in options) {
				if (key == "services") {
					toBeUsed[key] = Object.assign({}, serviceToBeUsed);
					options.services.forEach(function (service) {
						if (typeof service == "string") {
							toBeUsed[key][service] = service;
						} else if (service && (typeof service === 'undefined' ? 'undefined' : _typeof(service)) == "object") {
							for (var sKey in service) {
								if (!Lyte._keywords.is(sKey, "component")) {
									toBeUsed[key][sKey] = service[sKey];
								} else {
									Lyte.warn("Service cannot be used in any keywords of Lyte.");
								}
							}
			...
			if (serviceToBeUsed && !toBeUsed.services) {
				toBeUsed.services = serviceToBeUsed;
			}
			function Component() {}
			...
			customCrmComponent._serviceToBeUsed = toBeUsed.services;
				...
				if (tKey == "mixins") {
					...
					for (var serv in serviceToBeUsed) {
						if (Lyte.registeredServices.hasOwnProperty(serviceToBeUsed[serv])) {
							definition[serv] = Lyte.registeredServices[serviceToBeUsed[serv]];
						} else {
							Lyte.$.requiredServices(serv, serviceToBeUsed[serv], function (serviceData, serviceKey) {
								...
								customCrmComponent.component.prototype[serviceKey] = serviceData;
								customCrmComponent.compInstances.forEach(function (comp) {
									LyteComponent.set(comp.component.data, serviceKey, serviceData);
								});
							});
						}
					}
			...
			Lyte._registeredComponents[componentName] = _LC._registeredComponents[componentName] = customCrmComponent;
		},
		...
		"update": function update(object, property, value, fromStore, oldValue, setterScope, actualProperty, fromParent) {
			...
			if (value && typeof value !== "string" && typeof value !== "boolean" && typeof value !== "number") {
				...
				if (fromStore && Lyte.isRecord(object) || object && object.hasOwnProperty("_scp") && object._scp.size || dataDef && /^(object|array)$/.test(dataDef.type) && dataDef.watch == true) {
			...
			if (object._scp) {
				...
				if (oldValue) {
					object._scp.forEach(function (itm, id) {
						Lyte.removeNestScp(oldValue, id, undefined, Lyte.isRecord(value) || Array.isArray(value) && value.model && value.add ? value : undefined);
					});
				}
			}
			if (object._scpObj && object._scpObj.hasOwnProperty(property)) {
				Lyte.removeNestScp(oldValue, object._scpObj[property], undefined, Lyte.isRecord(value) || Array.isArray(value) && value.model && value.add ? value : undefined);
			}
		...
		"set": function set(object, property, value, fromStore, fromParent) {
			...
			if (object._scp && object._scp.size) {
				object._scp.forEach(function (obj, id) {
		...
		"aF": function aF() {
			...
			if (array._scp && /^(replaceAt|splice|removeAt|remove|insertAt)$/.test(functionName)) {
				array._scp.forEach(function (obj, id) {
					var rec,
					    attr,
					    wobj = {};
					var recObj = Lyte.nestScp[id];
          ...
          continue;
								...
								for (var _key29 in item) {
									...
									if (!isNaN(parsedKey) && _cond) {
										...
										this.affectChanges(item[_key29], undefined, _oldObject, undefined, array[parsedKey]);
          ...
          continue;
								...
								for (var _key30 in _item5) {
									...
									if (!isNaN(_parsedKey) && cond) {
										...
										this.affectChanges(_item5[_key30], undefined, _oldObject2, undefined, array[_parsedKey]);
									}
								}
							}
						}
          ...
          continue;
								...
								for (var _key31 in _item7) {
									...
									if (!isNaN(_parsedKey2) && _parsedKey2 >= _options2.firstIndex) {
										...
										this.affectChanges(_item7[_key31], undefined, _oldObject3, undefined, array[_parsedKey2]);
									}
								}
							}
						}
          	...
          	continue;
								...
								for (var _key32 in _item9) {
									...
									if (!isNaN(_parsedKey3) && _parsedKey3 >= _options3.firstIndex) {
										...
										this.affectChanges(_item9[_key32], undefined, array[_parsedKey3 + _options3.secondIndex], undefined, array[_parsedKey3]);
									}
								}
							}
						}
		...
		"callDeepObservers": function callDeepObservers(data, args, property) {
			var type = args.type,
			    self = this;
			data._scp.forEach(function (scp, id) {
				var pathArr = Object.keys(scp),
				    aPath;
				if (pathArr.length == 1) {
					aPath = pathArr[0];
				} else if (pathArr.length > 1) {
				...
				if (rec._bindings) {
					...
					for (var _i65 = 0; _i65 < objbind.length; _i65++) {
						...
						if (obj) {
							...
							if (property) {
								if (pathArr.length > 1) {
									...
									aPath = aPath ? aPath.concat("." + property) : property;
								}
							}
						...
						if (type == "array" && binding.length) {
							self.affectChanges(binding.length);
						}
					}
				}
			});
				...
				watch.forEach(function (wObj) {
					...
					if (remItems) {
						remItems.forEach(function (itm) {
							Lyte.removeNestScp(itm, wObj.id, undefined, wObj.reInit ? wObj.data : undefined);
						});
					}
					if (insItems) {
		...
		"get": function get(context, path, ac, cache) {
			...
			if (cache && cache.hasOwnProperty(path)) {
				...
				try {
					...
					for (var _i80 = 1; _i80 < _arr2.length; _i80++) {
						var locVal = _arr2[_i80];
	...
	Lyte.$.injectServices.component = function (key, name) {
		var registeredComponents = Lyte._registeredComponents;
		if (registeredComponents) {
			for (var comp in registeredComponents) {
				var component = registeredComponents[comp],
				    obj = {};
				obj[key] = name;
				component._serviceToBeUsed = Object.assign(obj, component._serviceToBeUsed);
				component.component.prototype[key] = Lyte.registeredServices[name];
				component.compInstances.forEach(function (compIns) {
					Lyte.Component.set(compIns.component.data, key, Lyte.registeredServices[name]);
				});
			}
		}
	};
...
var $Record = function $Record() {
	Object.assign(this, { inIDB: false, isUnloaded: false, isModified: false, isNew: false, isDeleted: false, isError: false, events: [], validatedOnCreate: true, error: {} });
};
/* Record Constructor
Steps Done: 
* Assign data
...
***
*/
var Record = function Record(modelName, data, opts) {
...
Object.defineProperties($Record.prototype, {
	...
	destroyRecord: {
		value: function value(customData, qP, delayPers) {
	...
	persist: {
		value: function value(obj) {
			...
			if (this.isNew) {
				store.$.toJSON(mName, rec, undefined, "create", partialObj);
			} else if (this.isModified) {
				var data = store.$.updateJSON(rec, this.model, this.isDirty());
...
var store = {
	...
	$: {
		...
		getFromCB: function getFromCB(type, name, key) {
			var scp = store[type],
			    obj = scp ? scp[name] : undefined;
			var result = obj ? obj[key] : undefined;
			while (result === undefined) {
				if (obj && obj.$super) {
					obj = obj.$super;
					result = obj ? obj[key] : undefined;
				} else if (name !== "application") {
					obj = scp ? scp.application : undefined;
		...
		extendService: function extendService(services, type, name, self, scope) {
			Lyte.extendService({ services: services, type: type, name: name, ins: self, callback: function callback(serv, key, name) {
					Lyte.$.lazyRegisterService(serv, { scope: scope, type: type, name: name, key: key });
				} });
		},
		...
		addDeepNest: function addDeepNest(record, extended, type) {
			var relatedRecord;
		...
		registerField: function registerField(model, key, field, obs) {
			...
			if (field.type == "observer") {
				...
				if (field.hasOwnProperty("watch") && field.watch == true && /^(array|object)$/.test(field.type)) {
		...
		idSerialize: function idSerialize(obj, rel, expose, partialObj, partialRef) {
			...
			if (Array.isArray(data)) {
				...
				if (data.$.isNew && expose != true && expose != "state") {
					...
					store.$.rSerialize(data, rel, bModel, model._pK, pkVal, expose, partial, partialRef);
				} else {
					if (polymorphic) {
						obj[relKey] = this.polymorphicToJSON(rel.forward, data);
		...
		removeSelfCircularReference: function removeSelfCircularReference(modelName, obj, expose, type, partialObj, partialRef) {
			var model = store.model[modelName],
		...
		toJSONObj: function toJSONObj(model, data, expose, type, partial) {
			var copyObj,
			    pkVal,
			...
			this.removeSelfCircularReference(modelName, copyObj, expose, type, partialMp, partial && partial.ref ? partial.ref : undefined);
			if (expose == "idb") {
		...
		toInsertData: function toInsertData(modelName, payLoad, saveParent) {
			var model = store.model[modelName];
			var data = this.insertIntoStore(modelName, payLoad[modelName], saveParent, true, undefined);
			model ? delete model.rel : undefined;
			return data;
		},
		insertIntoStore: function insertIntoStore(modelName, data, saveParent, stack, partialObj) {
			var ret;
			if (Array.isArray(data)) {
				ret = [];
				for (var i = 0; i < data.length; i++) {
		...
		removeFromStore: function removeFromStore(model, keys, fromStore, ignorePartial, delayPer, onlyRem) {
			var data = model.data;
			if (data.length == 0) {
				return;
			}
			...
			for (var i = 0; i < keys.length; i++) {
				...
				if (Object.keys(relations).length) {
					this.toDemolishRelation(model, index, ignorePartial, onlyRem, delayPer);
				}
				var deleted;
		...
		createAndRelate: function createAndRelate(fModel, bModel, data, key, val, rel, partial) {
			...
			if (!isComp && (typeof val === 'undefined' ? 'undefined' : _typeof(val)) == bModel.fieldList[bModel._pK].type) {
				...
				relatedRecord = this.insertIntoStore(bModel._name, val, undefined, undefined, newPartial);
			}
		...
		establishLink: function establishLink(forward, backward, data, relatedRecord, index, ignorePartial, isDuplicate) {
			...
			if (forward.relType == "belongsTo") {
				...
				var ret;
				if (!ignorePartial && forward.opts && forward.opts.serialize) {
			...
			if (backward.dummy) {
				...
				if (backward.relType == "belongsTo") {
					...
					if (!ignorePartial && backward.opts && backward.opts.serialize) {
						store.$.partialData(relatedRecord, bRelKey, data.$.pK, "added");
					}
				}
			}
			if (nest) {
				store.$.addDeepNest(relatedRecord, undefined, !ignorePartial ? "added" : undefined);
			}
			return true;
		},
		toDemolishRelation: function toDemolishRelation(model, index, ignorePartial, onlyRem, delayPers) {
			...
			for (var key in relations) {
				...
				for (var i = 0; i < rel.length; i++) {
					...
					if (relatedModel) {
						...
						this.toDemolishLink(model, record, relation, ignorePartial, onlyRem, delayPers);
					}
				}
			}
		},
		toDemolishLink: function toDemolishLink(model, record, relation, ignorePartial, onlyRem, delayPers) {
			...
			if (bRelation) {
				if (Array.isArray(records)) {
					for (var i = 0; i < records.length; i++) {
						this.demolishLink(record, priKey, records[i], bRelation.relKey, relation, bRelation, ignorePartial, undefined, onlyRem, delayPers);
					}
				} else if (Lyte.isRecord(records)) {
					this.demolishLink(record, priKey, records, bRelation.relKey, relation, bRelation, ignorePartial, undefined, onlyRem, delayPers);
				}
			}
		},
		...
		demolishLink: function demolishLink(record, priKey, relatedRecord, bRelKey, relation, bRelation, ignorePartial, ignoreAttrCheck, onlyRem, delayPers) {
									...
									if (rec.hasOwnProperty(key)) {
										...
										if (Array.isArray(data)) {
											data.forEach(function (itm, ind) {
												self.establishLink(item, bRel, rec, itm, undefined, true);
											});
										} else {
											self.establishLink(item, bRel, rec, rec[key], undefined, true);
										}
									}
									// if(rec.hasOwnProperty(key)){
									// 	bMod = store.modelFor(item.relatedTo);
		...
		isModelDataArray: function isModelDataArray(data) {
			if (Array.isArray(data) && data.model && data.model instanceof Model) {
				return true;
			}
		},
...
store.adapter = {
	$: {
		buildURL: function buildURL(type, method, modelName, key, snapshot, queryParams, actionName, customData, url) {
			...
			var ret = { method: method ? method : "" },
			    res;

			res = store.$.initCB("adapter", modelName, "headersForRequest", { args: [type, queryParams, customData, actionName, key] });
			if (res) {
		...
		get: function get(type, modelName, key, queryParams, cacheQuery, customData, cacheData) {
			...
			if (mdl) {
				...
				if (urlObj.method == "POST") {
					var res = store.$.initCB("serializer", modelName, "serialize", { args: [type, undefined, undefined, customData, modelName, queryParams] });
					if (res) {
				...
				return new Promise(function (resolve, reject) {
					var model = store.modelFor(modelName);
					var idbObj = model ? model.idb : undefined;
		...
		sendingData: function sendingData(modelName, data, urlObj, type, customData, snapshot) {
			...
			var res = store.$.initCB("serializer", modelName, "serialize", { args: [type, payload, snapshot, customData, modelName, qP] });
			if (res) {
				payload = res.data;
			}
			urlObj.data = payload;
		...
		handleAjax: function handleAjax(obj) {
			if (obj.url) {
				if (!obj.modelName) {
					Lyte.error("Please pass modelName param to store.ajax call");
					return Promise.reject("Please pass modelName param to store.ajax call");
				}
			}
			var urlObj = this.buildURL("ajax", obj.type, obj.modelName, obj.index, undefined, obj.queryParams, undefined, obj.customData, obj.url);
			urlObj.data = obj.data;
			var qP = urlObj.qP,
			    res = store.$.initCB("serializer", obj.modelName, "serialize", { args: ["ajax", urlObj.data, undefined, obj.customData, obj.modelName, qP, undefined] });
			if (res) {
				urlObj.data = res.data;
			}
			return this.handleRequest(urlObj, obj.modelName, undefined, "ajax", undefined, obj.modelName, undefined, undefined, undefined);
		},
		...
		handleSuccess: function handleSuccess(modelName, type, xhr, data, urlObj, resolve, resp, respObj, partialObj, reject, key, customData, actionName, partialRef) {
			...
			return this.otherParseRequestPromise(resp, modelName, type, data, urlObj, xhr, partialObj, batchIndex, batch, resolve, reject, customData, partialRef);
		},
		...
		updateIDB: function updateIDB(modelName, type, data) {
			...
			var model = store.model[modelName];
			if (!model || !model.hasOwnProperty("idb") || model.idb && !model.idb.hasOwnProperty("queryCache")) {
				return;
			}
			var q = store.$.idbQ2[modelName] = store.$.idbQ2[modelName] || [];
			if (data) {
		...
		mergeResponse: function mergeResponse(data, model, response, pK, partialObj, cPersist) {
			...
			store.adapter.$.removeDirtyStack(data);
			store.$.removeDeepNest(data);
		},
		removeDirtyStack: function removeDirtyStack(data) {
			data.$._attributes = {};
		...
		handleResults: function handleResults(results, partialRef) {
			if (Array.isArray(results)) {
				var self = this;
				results.forEach(function (itm, idx) {
					var refKey = partialRef.refKey;
					var refId = itm[refKey],
					    refObj = partialRef.get(refId);
					if (refObj) {
						self.mergePartialObj(refObj, undefined, itm, true);
					}
				});
			} else {
				Lyte.warn("results expected be a flat structure of type array, instead it was " + (typeof results === 'undefined' ? 'undefined' : _typeof(results)));
			}
		},
		...
		mergePartialObj: function mergePartialObj(partialObj, data, response, doMerge, cPersist) {
			...
			if ((partialObj.$.type == "added" || data && data.$.isNew == true) && isRec) {
				mergeDone = this.mergeNewRecord(partialObj, data, response, doMerge, cPersist);
			} else if ((partialObj.$.type == "modified" || partialObj.$.type == "related" || data && data.$.isModified == true) && isRec) {
  ...
  */
		...
		otherParseRequestPromise: function otherParseRequestPromise(resp, modelName, type, data, urlObj, xhr, partialObj, batchIndex, batch, resolve, reject, customData, partialRef) {
...
if (Lyte.$.injectServices) {
	Lyte.$.injectServices.adapter = Lyte.$.injectServices.serializer = function (key, name, type) {
		var cls = store[type];
		var obj = {};
		obj[key] = name;
		for (var cName in cls) {
			var ins = cls[cName];
			store.$.extendService([obj], type, cName, ins, store);
		}
	};
}
	...
	$: {
		buildJSON: function buildJSON(modelName, type, payLoad, id, status, urlObj, customData, qP) {
			var headers = urlObj ? urlObj.headers : undefined,
			    realData = payLoad;
			var res = store.$.initCB("serializer", modelName, "normalizeResponse", { args: [modelName, type, realData, id, status, headers, urlObj ? urlObj.qP : undefined, customData] });
			if (res) {
				realData = res.data;
			}
			var changed = false,
		...
		normalizeResponse: function normalizeResponse(modelName, type, payLoad, id, status, urlObj, customData, opts) {
			var headers = urlObj ? urlObj.headers : undefined,
			    realData = payLoad;
			var res = store.$.initCB("serializer", modelName, "normalizeResponse", { args: [modelName, type, realData, id, status, headers, urlObj ? urlObj.qP : undefined, customData, opts] });
			if (res) {
				realData = res.data;
			}
			return realData;
...
Object.defineProperties(store, {
	...
	batch: {
		value: function value(method) {
			return new Promise(function (resolve, reject) {
				...
				var res = store.$.initCB("serializer", "application", "serialize", { args: ["batch", payLoad] });
				if (res) {
					payLoad = res.data;
				}
				var xhr = new XMLHttpRequest();
	...
	pushPayload: {
		value: function value(modelName, data, deserialize, index) {
			var model = store.modelFor(modelName);
			if (!model) {
				Lyte.warn("LD02", "Model", modelName);
				return;
			...
			if (result && model) {
				...
				for (var key in rels) {
					...
					relArr.forEach(function (rel) {
						var key = rel.relKey;
						var type = rel.relType;
						if (type == "hasMany" && result.hasOwnProperty(key) && Array.isArray(result[key]) && result[key].length) {
							var arr = result[key],
							    newArr = [];
							arr.forEach(function (itm) {
								if (Lyte.isRecord(itm)) {
							...
							result[key] = newArr;
						} else if (type == "belongsTo" && result.hasOwnProperty(key) && Lyte.isRecord(result[key])) {
			...
			data = store.$.insertIntoStore(modelName, result, true, undefined, undefined, index);
			model ? delete model.rel : undefined;
			return data;
		}
	},
 ...
 */
	findRecord: {
		value: function value(modelName, key, queryParams, cacheQuery, cacheData, customData, index) {
			if (key == undefined) {
				...
				if (data) {
					if (!fromCache) {
						...
						if (!Lyte.isRecord(data)) {
							...
							if (!isEmpty) {
								var record = store.$.insertIntoStore(modelName, payLoad, true, true, undefined);
