...
_observedAttributes :["ltPropScope","ltPropHandler","ltPropMarginLeft","ltPropMarginTop","ltPropUnitX","ltPropUnitY","ltPropResizeDirection","ltPropFloat","ltPropDirection","ltPropUndo","ltPropResize","ltPropBestfit","ltPropMinUnitX","ltPropMinMarginLeft","ltPropBestfitClass","ltPropFreezeMode","ltPropDefaultLength","ltPropDefaultHeight","ltPropDefaultMinLength","ltPropDefaultMinHeight","ltPropDefaultMaxHeight","ltPropDefaultMaxLength","ltPropColumnMode","ltPropColumn","ltPropPrevent","ltPropGridLength","ltPropBestfitType","ltPropForcedReposition","ltPropSquareGrid","ltPropGridSpaceColor","ltPropHitBottom","ltPropGridSelectionClass","ltPropVisibleBoundary","ltPropVisible","ltPropCheckCurrentPosition","ltPropContainment","ltPropMaxGridHeight","ltPropScrollElement","ltPropIgnoreDrag","gridLength","ltPropGridHeight","ltPropMarginLeftCopy","ltPropUnitxCopy","lyteGridStack","elements","oriNode","xElements","yElements","iniData","lyteQuerySelector","direction"],
    ...
    windowResize : function( evt ){
        ...
        __window._gridResize = setTimeout(function(){
            ...
            if( data.ltPropGridLength != void 0 && !data.ltPropForcedReposition ){
...
additionalCheck : function( element, height, topElements1 ){
        ...
        if( ori_data.length > 1 ) {
            elem2 = elem2.concat( xElements[ __x + 1 ] );
            elem2 = elem2.concat( xElements[ __x + __length - 1 ] );
        } else {
            elem2 = elem2.concat( this.similarData( xElements[ __x ], xElements[ __x + 1 ] ) );
        } 
...
// after rendering properties
    ...
    updateValue : function( grid_element, value ){
        ...
        new_pos = is_resize ? {} : this.emptySpaceFind( new_obj, index )[ 0 ],
     ...
     emptySpaceFind : function(objj, flagg, mappArray1, flag2){
        ...
        if( !flagg ){
            ...
            for( i = 0; i < __length; i++ ) {   
                ...
                if(  cur_data.x != void 0 && cur_data.y != void 0 ) {
                    for( var k = cur_data.x; k < ( cur_data.x + cur_data.length); k++ ) {
                        var __cur = mappArray[ k ];
                        for( var m = cur_data.y; m < ( cur_data.y + cur_data.height ); m++ ) {
    ...
    modified_move : function( ev ){
        ...
        if( __selected.length == 1 ){
            if( __this.modified_scrollcheck( ev, node, _sW, _oW, _sL, 'scrollLeft', __left, 'width', '__clientX', node_bcr, _ww ) ){
                __this.modified_scrollcheck( ev, node, _sH, _oH, _sT, 'scrollTop', 'top', 'height', '__clientY', node_bcr, _wh );
            }
    ...
    modified_scrollcheck : function( evt, node, scrollWidth, offsetWidth, scrollLeft, _sL, left, width, __clientX, node_bcr, _ww ){
        ...
        if( this.data.ltPropScrollElement ){
            if( left == "top" ){
                if( Math.max( 0, scroll_bcr.top ) > node_bcr.top && scrollLeft ){
                    fact = -1;
                } else if( Math.min( _ww, scroll_bcr.top + scroll_bcr.height ) < ( node_bcr.top + node_bcr.height ) ){
                    fact = 1;
                }
            } else {
                if( Math.max( 0, scroll_bcr.left ) > node_bcr.left && scrollLeft ){
                    fact = -1;
                } else if( Math.min( _ww, scroll_bcr.left + scroll_bcr.width ) < ( node_bcr.left + node_bcr.width ) ){
                    fact = 1;
                }
            }
            modified_scroll = fact;
        } else {
            if( __left < scrollLeft ){
                if( scrollLeft ){
                    fact = -1;
                }
            } else if( __left + __width > offsetWidth ){
        ...
        if( fact ){
            ...
            var __new = Math.min( Math.max( 0, ( scrollLeft + 10 * fact ) ), scrollWidth - offsetWidth );
    ...
    modified_vertical : function( item, is_content, xInc, yInc, frm_recursice, horizontal ){
        ...
        if( yInc > 0 && is_content ){
            __arr = this.multipleRemoval( left_bottom.concat( right_bottom ).concat( mid ) );
            fn = horizontal || 'modified_toptobottom';
        } else if( yInc < 0 || !is_content ){
            __arr = this.multipleRemoval( left_top.concat( right_top ).concat( mid ) );
        ...
        if( __arr.length ){
            return this[ fn ]( item, __arr, xInc, yInc, frm_recursice );
        }
    },
    ...
    modified_toptobottom : function( item, elems, xInc, yInc ){
        var __data = this.data,
        data = __data.lyteGridStack,
        ret,
        modified = [],
        ...
        elems.every( function( __item ){
            ...
            if( hgt ){
                ...
                exp_y = Math.max( new__y + cur_data.height, exp_y );
                ...
                exp_y = Math.max( exp_y, cur_data.y + cur_data.height );
        ...
        if( ret ){
            ...
            if( this.modified_vertical( item, true, xInc, -1, true ) ){
                this.flush_modified( modified, elements );
                return true;
            }
    ...
    modified_bottomtotop : function( item, elems, xInc, yInc, frm_recursice ){
         var __data = this.data,
        data = __data.lyteGridStack,
        ret,
        modified = [],
        ...
        elems.every( function( __item ){
            ...
            if( max_height < current_bottom + cur_data.height ){
                ...
                ret = this.modified_vertical( node, true, xInc, yInc, true );
            }
