...
_observedAttributes :["shouldDisplayIcon","changeItToInvoke","dummy1","dummy2","savedPositions","ltPropRemoveMultiple","ltPropYield","ltPropType","ltPropTabindex","ltPropShow","ltPropFreeze","ltPropOptions","ltPropUserValue","ltPropSystemValue","ltPropPosition","ltPropIconClass","ltPropSelected","ltPropCallout","ltPropPlaceholder","ltPropDisabled","ltPropHover","ltPropNoResult","ltPropMaxCount","ltPropInputClass","ltPropBoundary","pos","secondaryPosition","firePos","ltPropAjaxRequest","firstRequest","userValue","sysValue","ltPropDisplayValue","ltPropDisabledList","ltPropAnimate","ltPropSetPos","ltPropTooltip","ltPropBoxClass","ltPropIsOpen","ltPropFocus","ltPropFixPositionOnOpen","ltPropForcePlaceholder","ltPropShowEmptyMessage","ltPropBoxButtonWidth","ltPropPreventScroll","ltPropScope","ltPropAnimateBox","ltPropTooltipClass","ltPropDisplayList","ltPropSelectedList","ltPropItemSearchType","ltPropFocusOnClose","ltPropButtonClass","ltPropDisableItemTooltip","ltPropPreventParentScroll","ltPropShowRemoveIcon","ltPropAriaButton","ltPropAriaBox","ltPropAriaBody"],
	...
	didConnect: function() {
		...
		drop.mutobserver = new MutationObserver( function( mutations ) {
			...
			if( this.getData( 'multiple' ) ) {
				...
				for( i = 0; i < mutations.length; i++ ) {
					...
					/* 
					 * Vikash issue fix. Need to discuss with Anantha anna. 
					 * In that tester's machine childList and style change of lyte-drop-box are in a single mutation argument array
					 * So final value of stylemutations is true. So notselected value can't become true
					 * This is a temporary fix - Pk
					 */

					else if( mutations[ i ].type == 'childList' ){
						stylemutations = false;
						break;
					}
		...
		this.getDropBox().addEventListener( 'mousemove', function( event ) {
			...
			if( cur ) {
				...
				this.getAriaButton().setAttribute( 'aria-activedescendant', cur.getAttribute( 'id' ) );
			}
	...
	*/
	...
	toggleCurrent: function( node ) {
		...
		this.getAriaButton().setAttribute( 'aria-activedescendant', node.getAttribute( 'id' ) );
	},
	 ...
	 setHighlight: function( elems, node ) {
		...
		if( elems[ i ] && elems[ i ] === node ) {
			...
			if( i != elems.length ) {
				...
				this.getAriaButton().setAttribute( 'aria-activedescendant', elems[ i ].getAttribute( 'id' ) );
			}
			else {
				...
				if( j != -1 ) {
					...
					this.getAriaButton().setAttribute( 'aria-activedescendant', elems[ j ].getAttribute( 'id' ) );
				}
			}
	...
	if(
		...
		if( 
			...
			if( elem ) {
				...
				component.getAriaButton().setAttribute( 'aria-activedescendant', elem.getAttribute( 'id' ) );
				return ;
			}
		}
		...
		// Key up was pressed
			...
			if( i != -1 ) {
				...
				elems[ i ].classList.add( 'lyteDropdownSelection' );
				component.getAriaButton().setAttribute( 'aria-activedescendant', elems[ i ].getAttribute( 'id' ) );
	...
	if( 
		...
		if( open.length > 0 && drp ) {
			...
			if( pos != undefined ) {
				...
				drp.component.getAriaButton().setAttribute( 'aria-activedescendant', children[ pos ].getAttribute( 'id' ) );
