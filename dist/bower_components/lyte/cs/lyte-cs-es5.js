  ...
  */
      ...
      value: function afterConnected() {
        ...
        if (this._initProperties) {
          ...
          for (var _key7 in initProperties) {
            var actVal = void 0;
            if (compInstance.__data[_key7] && compInstance.__data[_key7].type !== _LC.getDataType(initProperties[_key7]) && (initProperties[_key7] !== undefined || compInstance.__data[_key7].type === "boolean")) {
      ...
      value: function renderNodes(toAppend, dynamicNodes, helperNode, options, establishBindings, returnVal, templateContent) {
        ...
        for (var _i7 = 0; _i7 < dynamicNodes.length; _i7++) {
          ...
          if (type === "componentDynamic") {
            ...
            this.bindNode(dynamicN, undefined, helperNode, options, dynamicNodes[_i7], processLast, establishBindings);
          } else if (type === "attr") {
      ...
      value: function processArgs(scope, dynN, dynamicValues, event, node, newCompile) {
        ...
        for (var _i35 = 0; _i35 < args.length; _i35++) {
          if (args[_i35] && args[_i35].type) {
            this.internalHelpers(scope, args, _i35, dynamicValues, event, node, newCompile);
          } else {
            if (!this.isEmptyString(args[_i35])) {
              if (args[_i35].startsWith("'") && args[_i35].endsWith("'")) {
                ...
                if (args[_i35] === "event" && event) {
                  ...
                  args[_i35] = _LC.get(scope.component.data, args[_i35], dynamicVals);
                }
    ...
    "affectChanges": function affectChanges(item, contextAlreadySwitched, oldValue, setterScope, newValue) {
      ...
      if (Array.isArray(item)) {
        ...
        for (var _key35 in item) {
          this.affectChanges(item[_key35], true, oldValue ? oldValue[_key35] : oldValue, setterScope, newValue ? newValue[_key35] : newValue);
        }
      }
    },
  ...
  function getHelperArgs(str, newCompile) {
    ...
    for (var _i81 = 0; _i81 < str.length; _i81++) {
      if (!stack.length && str.charAt(_i81) === ",") {
        var _toPush2 = str.substr(from, _i81 - from);
        ...
        args.push(_toPush2);
        from = _i81 + 1;
      } else if (str.charAt(_i81) === "(") {
        if (stack[stack.length - 1] != "'" && stack[stack.length - 1] != "\"") {
          stack.push(str.charAt(_i81));
          lastPushed = str.charAt(_i81);
        }
      } else if (str.charAt(_i81) === ")") {
        if (stack[stack.length - 1] === "(") {
          stack.pop();
        }
      } else if (str.charAt(_i81) === "'" && str.charAt(_i81 - 1) !== "\\") {
        if (stack[stack.length - 1] === "'") {
          stack.pop();
        ...
        case "attr":
          {
            ...
            for (var _i82 = 0; _i82 < node.attributes.length; _i82++) {
              if (node.attributes[_i82].nodeValue.indexOf("{{") !== -1) {
                var val = node.attributes[_i82].nodeValue;
                ...
                if (actObj && (actObj.name === "action" || actObj.name === "method") && /^(onfocus|onfocusin|onfocusout|onresize|onscroll|onclick|ondblclick|onmousedown|onmouseup|onmousemove|onmouseover|onmouseout|onchange|onselect|onsubmit|onkeydown|onkeypress|onkeyup|oncontextmenu)$/.test(node.attributes[_i82].name)) {
                  attr[node.attributes[_i82].name.substr(2)] = { name: node.attributes[_i82].name.substr(2), camelCase: _LC.String.toCamelCase(node.attributes[_i82].name.substr(2)), helperInfo: actObj, newHelperInfo: getHelper(actValue, true), globalEvent: true };

                  var actArgs = _Lyte.deepCopyObject(actObj.args);
                  var actName = actArgs.splice(0, 1)[0];
                  actName = actName.startsWith("'") ? actName.replace(/'/g, '') : actName;
                  var actString = getArgString(actName, actArgs);
                  node.setAttribute(node.attributes[_i82].name.substr(2), componentName + " => " + actString);
                  if (dynNewCompile) {
                    dynNewCompile.setAttribute(node.attributes[_i82].name.substr(2), componentName + " => " + actString);
                  }
                  toBeRemoved.push(node.attributes[_i82].name);
                } else {
                  if (actObj || actValue) {
                    var attrToPush = {};

                    if (node.attributes[_i82].name.startsWith("lbind:")) {
                      toBeRemoved.push(node.attributes[_i82].name);
                      toBeAdded.push({ "name": node.attributes[_i82].name.substring(6), "value": node.attributes[_i82].nodeValue });
                      attrToPush.isLbind = true;
                      attrToPush.name = node.attributes[_i82].name.substring(6);
                      attrToPush.camelCase = _LC.String.toCamelCase(attrToPush.name);
                    } else {
                      attrToPush.name = node.attributes[_i82].name;
                      attrToPush.camelCase = _LC.String.toCamelCase(attrToPush.name);
                    }
                    if (actObj) {
            ...
            if (toBeRemoved.length) {
              for (var _i83 = 0; _i83 < toBeRemoved.length; _i83++) {
                node.removeAttribute(toBeRemoved[_i83]);
                if (dynNewCompile) {
                  dynNewCompile.removeAttribute(toBeRemoved[_i83]);
                }
              }
            }
            if (dynNewCompile) {
            ...
            if (toBeAdded.length) {
              for (var _i84 = 0; _i84 < toBeAdded.length; _i84++) {
                node.setAttribute(toBeAdded[_i84].name, toBeAdded[_i84].value);
              }
            }
          }
          break;
  ...
  function getArgString(name, array) {
    ...
    for (var _i85 = 0; _i85 < array.length; _i85++) {
      if (array[_i85] && _typeof(array[_i85]) === "object") {
        array[_i85] = getArgString(array[_i85].value.name, array[_i85].value.args);
      }
    }
    if (name) {
...
Object.defineProperties($Record.prototype, {
  ...
  rollBackAttributes: {
    value: function value(attr) {
      ...
      var func = function func(item, mKey) {
...
var store = {
  ...
  $: {
    ...
    // 8 
    // [ 0,3,4,6,7,8,9,10,11 ]
    getDefaultVal: function getDefaultVal(rec, field, defVal) {
