<template tag-name="lyte-dual-listbox" role="listbox">
  <div class="lyteListboxWrapper">
    <div class="lyteListboxLeftPanel lyteListboxPanels" id="lyte_listbox_left_panel" aria-label="left panel" aria-multiselectable=true aria-orientation="vertical" onfocus="{{action('focusPanel' , this)}}" onfocusout="{{action('blurPanel' , this)}}" aria-activedescendant="{{listboxLeftAriaDescendant}}">
      <lyte-search class="lyteListBoxSearch" lt-prop-class="lyteListboxInput" lt-prop-query-selector='{"scope" : ".{{ltPropSearchScope}}", "search" : ".lyteListBoxLeftElement", "target" : ".lyteListBoxLeftElement"}' lt-prop-check-from-parent=true lt-prop-component="duallistbox" on-search="{{method('onSearch')}}"></lyte-search>
      <div class="lyteListboxleftElementWrap lyteListboxSortablePanels">
        ...
        <div class="lyteListBoxLeftWrap {{if(item.childrenList.length===0,'lyteLBoxDisabledParent','')}} {{if(childitem.required,'lyteListBoxRequiredParent','')}}">
          <div class="lyteListBoxLeftElement lyteListBoxLeftParentElement lyteLeftLvl1 {{if(item.required,'lyteListBoxRequiredItem','')}} {{if(item.childrenList.length===0,'lyteLBoxDisabledParent','')}}" ondblclick="{{action('moveElementRight' , this , 'doubleClick')}}"
            onmouseup="{{action('elementFN' , this , event)}}" index="{{lyteListBoxIndex(index)}}" dom-index="" id="" role="option" aria-selected=false aria-owns="{{lyteListBoxChildIndexId(index)}}">
          ...
          <div class="lyteLBLeftChildWrap" id="{{lyteListBoxChildIndexId(index)}}">
            ...
            <div class="lyteListBoxLeftWrap lyteLeftLvl2Wrap {{if(childitem.required,'lyteListBoxRequiredParent','')}}">
              <div class="lyteListBoxLeftElement lyteLeftLvl2 {{if(childitem.required,'lyteListBoxRequiredItem','')}}" ondblclick="{{action('moveElementRight' , this , 'doubleClick')}}" onmouseup="{{action('elementFN' , this, event)}}" onmousedown="{{action('elementFN' , this, event)}}" index="{{lyteListBoxIndex(ind , index)}}"
                dom-index="" id="" role="option" aria-selected=false>
                <lyte-yield class="lyteListboxYield" yield-name="leftWidget" left-widget-value={{childitem}}></lyte-yield>
        ...
        <div class="lyteListBoxLeftWrap {{if(item.required,'lyteListBoxRequiredParent','')}}">
          <div class="lyteListBoxLeftElement lyteLeftLvl1 {{if(item.required,'lyteListBoxRequiredItem','')}}" ondblclick="{{action('moveElementRight' , this , 'doubleClick')}}" onmouseup="{{action('elementFN' , this, event)}}" onmousedown="{{action('elementFN' , this, event)}}" index="{{lyteListBoxIndex(index)}}"
            dom-index="" id="" role="option" aria-selected=false>
            <lyte-yield class="lyteListboxYield" yield-name="leftWidget" left-widget-value={{item}}></lyte-yield>
            <lyte-lb-add class="lyteLbAdd" lyte-if={{!item.required}} onclick="{{action('addElementToRight' , this)}}"></lyte-lb-add>
          </div>
    ...
    <div class="lyteListboxRightPanel lyteListboxPanels lyteListboxSortablePanels" tabindex=0 id="lyte_listbox_right_panel" aria-label="right panel" aria-orientation="vertical" onfocus="{{action('focusPanel' , this)}}" onfocusout="{{action('blurPanel' , this)}}" aria-activedescendant="{{listboxRightAriaDescendant}}">
      <% ltPropRightData.forEach(function(item, index){%>
      <div class="lyteListBoxRightWrap {{if(item.required,'lyteListBoxRequiredParent','')}}">
        <div class="lyteListBoxRightElement lyteRightLvl1 {{if(item.required,'lyteListBoxRequiredItem','')}}" ondblclick="{{action('moveElementLeft' , this , 'doubleClick')}}" onmouseup="{{action('elementFN' , this, event)}}" onmousedown="{{action('elementFN' , this, event)}}" index="{{lyteListBoxIndex(index)}}"
