<template tag-name="lyte-combobox">
	<lyte-dropdown class="{{ltPropDropdownClass}} {{if(ifEquals(ltPropType,'buttonSearch'),'lyteComboboxButtonSearch','lyteComboboxBoxSearch')}} {{if(isSearch,'lyteComboboxSearchPresent')}}" lt-prop = {{stringify(ltPropDropdown)}} lt-prop-selected = {{ltPropDropdownSelected}} lt-prop-options={{ltPropOptions}} lt-prop-disabled-list={{ltPropDisabledList}} on-option-selected="{{method('optionSelect')}}" on-show="{{method('dropBoxOpen')}}"  on-animation-end="{{method('onAnimateEnd')}}" on-key-navigation="{{method('keyNavigation')}}" on-before-show="{{method('beforeDropboxShow')}}" on-hide="{{method('hideDropbox')}}" on-before-hide="{{method('beforeHideDropbox')}}" on-scroll="{{method('scrollDropBox')}}"
			...
			<%if(! ltPropYield ) {%>
		  	  ...
		  	  <%} else {%>
			  	  		<lyte-drop-button > 
			  	  			...
			  	  			<% if( isSearch && ltPropType == 'buttonSearch' ){ %>
						  	  	<lyte-input class="lyteComboboxBtnSearchInput" lt-prop-close-icon=true lt-prop-aria={{ltPropAria}} lt-prop-appearance ={{ltPropDropboxSearchAppearance}} lt-prop-type="search" lt-prop-class = "lyteComboboxBtnSearchOrigIpt" lt-prop-placeholder = {{ltPropSearchplaceholder}}  on-value-change="{{method('searchList')}}" on-clear="{{method('onSearchClear')}}"></lyte-input>
						  	<% } %>
			  	  		</lyte-drop-button>
			  	  		<lyte-drop-box class="{{ltPropBoxClass}}">
			  	  			<% if( isSearch && ltPropType == 'boxSearch' ){ %>
