<template tag-name="lyte-voicenote">
	...
	<% if(ltPropVolCtrlEnabled){ %>
		<div class="lyteVoiceNoteVolumeController">
			<span class="lyteVoiceNoteVolumeIcon" onclick="{{action('onMute')}}"></span>
			<div class="lyteVoiceNoteVolumeSlider">
				<lyte-multislider lt-prop-max = '1' lt-prop-fill-color = '#ddd' lt-prop-height='5px' lt-prop-yield=true lt-prop-width='60px' lt-prop-handler='lyteCircle' lt-prop-value = '[ { "value" : {{ltPropVolume}}, "min" : 0, "max" : 1 } ]' on-change="{{method('setVolume')}}" lt-prop-color = '["#61A6E8" ]'></lyte-multislider>
			</div>
		</div>		
	<% }%>
	<audio onpause="{{action('pause')}}" onplay="{{action('play')}}" onvolumechange="{{action('changeVolume', event)}}" onloadedmetadata="{{action('meta', event)}}" ontimeupdate="{{action('update',event)}}" preload="{{ltPropPreload}}" onprogress="{{action('progress',this)}}">
		...
		<%ltPropSrc.forEach( function( item, index ){%>
			<source src={{item.src}} type='{{item.type||"audio/mpeg"}}' label = {{item.label}}>
		<%})%>
	</audio>
	...
	<time class="lyteVoiceNoteElapsedTime" datetime={{elapsedFormat}} lyte-if="{{elapsedTime}}">{{elapsedTime}}</time>
