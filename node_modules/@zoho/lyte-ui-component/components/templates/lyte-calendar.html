                    ...
                    <div class='lyteCalendarNavigator'>
                        <% if(ifNotEquals(ltPropHeaderType,"dropdown")){ %>
                            <template is="if" value="{{ltPropYear}}">
                                <span role="button" case="true" tabindex="0" class="lyteCalNav lyteCalyearNavLft" onclick="{{action('previous','Y',event)}}" onkeydown={{action('previousOnKeys','Y',event)}}>
                        ...
                        <% if(ifNotEquals(ltPropHeaderType,"dropdown")){ %>
                            <template is="if" value="{{ltPropYear}}">
                                <span case="true" role="button" class="lyteCalNav lyteCalyearNavRgt" tabindex="0" onkeydown={{action('nextOnKeys','Y',event)}} onclick="{{action('next','Y',event)}}">
                    ...
                    <% if( ltPropHeaderType !== 'drilldown' || ( ltPropHeaderType === 'drilldown' && viewType === 'dateView' ) ) { %>
                        <div role="row" class="lyteCalTableRowHeader">
                            <template is='for' items={{daysOfWeek}} item="day" indexval="idod">
                                <div role="columnheader" aria-label={{day.title}} class="lyteCalTableCellHeader" id={{day.id}}>{{lyteUiI18n(day.day)}}</div>
                            </template>
                        </div>
                    <% } %>
                    <div class="lyteCalTableRowGroup">
                        <% if( ltPropHeaderType !== 'drilldown' ) { %>
                            <template is='for' items={{matrix}} item="vector" index="rowid">
                                <template is="if" value="{{lyteUiCheckEmpty(vector)}}">
                                    <template case="false">
                                        <div class="lyteCalTableRow" role="row">
                                            <template is='for' items={{vector}} item="date" index="cellid">
                                                <template is="if" value="{{lyteUiCheckInRange(ltPropMinDate,ltPropMaxDate,date.val,ltPropFormat)}}">
                                                    <template case="true">
                                                        <template is="if" value={{date.emptyBlock}}>
                                                            <template case="true">
                                                                <div role="gridcell" class="lyteCalEmpty"></div>
                                                            </template>
                                                            <template case="false"> 
                                                                <div role="gridcell" id={{date.id}} aria-labelledby="{{concat(date.id, ' ', headerId, ' ', daysOfWeek[cellid]['id'])}}" tabindex="-1" data-date={{date.val}} onkeydown="{{action('performNavigation',event)}}" onclick="{{action('dateSelected',event)}}" class={{date.clsname}}><span class="lyteCalDateSpan">{{date.date}}</span></div>
                                                            </template>
                                                        </template>
                                                    </template>
                                                    <template case="false">
                                                        <template is="if" value={{date.emptyBlock}}>
                                                            ...
                                                            <template case="false">
                                                                <div role="gridcell" tabindex="-1" id={{date.id}} aria-labelledby="{{concat(date.id, ' ', headerId, ' ', daysOfWeek[cellid]['id'])}}" aria-disabled="true" onkeydown="{{action('performNavigation',event)}}"  data-date={{date.val}}  class={{date.clsname}}><span class="lyteCalDateSpan">{{date.date}}</span></div>
                                                            </template>
                                                        </template>
                                                    </template>
                                                </template>
                                                    ...
                                                    <template is="if" value="{{lyteUiCheckInRange(ltPropMinDate,ltPropMaxDate,date.val,ltPropFormat)}}">
                                                        <template case="true">
                                                            <template is="if" value={{date.emptyBlock}}>
                                                                ...
                                                                <template case="false">
                                                                    <div role="gridcell" tabindex="-1" id={{date.id}} aria-labelledby="{{concat(date.id, ' ', headerId, ' ', daysOfWeek[cellid]['id'])}}" onkeydown="{{action('performNavigation',event)}}" onclick="{{action('dateSelected',event)}}" class={{date.clsname}} data-date={{date.val}}><span class="lyteCalDateSpan">{{date.date}}</span></div>
                                                                </template>
                                                            </template>
                                                        </template>
                                                        <template case="false">
