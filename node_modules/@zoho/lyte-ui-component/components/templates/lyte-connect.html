<template tag-name="lyte-connect" onmousedown="{{action('mousedown',event)}}" ontouchstart = "{{action('mousedown',event)}}" onscroll="{{action('scroll',event)}}" onkeydown="{{action('keydown',event)}}" tabindex="0">
	<div class="lyteConnectWrapper">
		<%ltPropData.forEach( function( item, index ){
			if( item.children ){%>
				<lyte-connect-item lt-prop-anywhere = {{ltPropAnywhere}} lt-prop-default-anchors = {{ltPropDefaultAnchors}} id = "{{ltPropIdPrefix}}{{item.id}}" class="lyteConnectGroupShape {{item.class}}" data-index = {{index}} lt-prop-item = "{{details[item.id]}}" style = "{{construct_style(item.position)}}">
					<%item.children.forEach(function( _item, _index ){%>
						<lyte-connect-item lt-prop-anywhere = {{ltPropAnywhere}} lt-prop-default-anchors = {{ltPropDefaultAnchors}} id = "{{ltPropIdPrefix}}{{_item.id}}" class = "lyteConnectInnerItem {{_item.class}}" data-index = {{_index}} style = "{{construct_style(_item.position)}}" onmouseenter = "{{action('mouseenter',this,event)}}" onmouseleave = "{{action('mouseleave',this,event)}}" lt-prop-item = "{{details[_item.id]}}">
							<lyte-yield yield-name = "connection" connection = {{_item}}></lyte-yield>
						</lyte-connect-item>
					<%})%>
				</lyte-connect-item>
			<%}else{%>
				<lyte-connect-item lt-prop-anywhere = {{ltPropAnywhere}} lt-prop-default-anchors = {{ltPropDefaultAnchors}} id = "{{ltPropIdPrefix}}{{item.id}}" class = {{item.class}} data-index = {{index}} style = "{{construct_style(item.position)}}" onmouseenter = "{{action('mouseenter',this,event)}}" onmouseleave = "{{action('mouseleave',this,event)}}" lt-prop-item = "{{details[item.id]}}">
