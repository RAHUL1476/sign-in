	...
	<% if( ltPropAlignment !== 'vertical' ) {%>
		...
		<% } else { %>
			<% auxArray.forEach(function(item,index){ %>
				<lyte-nav-item data-value={{item[ltPropSystemValue]}} lyte-shortcut={{lyteUiGetValue(item,'shortcut')}}>{{item[ltPropUserValue]}}</lyte-nav-item>
			<% }) %>
		<% } %>
		
		<% if(ltPropType === 'collapse' ){ %>
			<% if(menuContent.length > 0 || showMenu){%>
				<span class={{ltPropContainerClass}} role="button" aria-expanded="false" aria-labelledby={{randomId}} tabindex="0">
			...
			<lyte-menu 
				...
				on-menu-click={{method('itemSelected')}}
