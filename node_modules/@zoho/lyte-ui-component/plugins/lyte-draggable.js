	...
	if(lyteDomObj){
		...
		lyteDomObj.prototype.draggable = function(object) {
				...
				if(_handleElement){
					...
					if(data && data._isDown){
						...
						if(!data._isMoved){
							...
							if(data.onDragStart){
								data.returnVal = onDragStart(data,_handleElement,_placeholder,event);
							}
							if(data.scrollDivY){
						...
						//Callback fired
						if(data.returnVal && data.onDrag){
							data.returnVal = onDrag(data,_handleElement,event);
						}
			...
			var onDrag = function(data, _handleElement,event){
				returnVal = data.onDrag(data._element,_handleElement,event,data._origin);
				return (returnVal == undefined) ? true : returnVal;
			}
